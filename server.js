...
// ===========================================
// پوشه voice
const VOICE_UPLOADS = path.join(UPLOADS_DIR, 'voice');
if (!fs.existsSync(VOICE_UPLOADS)) fs.mkdirSync(VOICE_UPLOADS, { recursive: true });

// ذخیره وضعیت تایپ در حافظه
...
const voiceStorage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, VOICE_UPLOADS),
  filename: (req, file, cb) => {
    cb(null, `voice-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.webm`);
  }
});
const uploadVoice = multer({
  ...
  limits: { fileSize: 10 * 1024 * 1024 },
  fileFilter: (req, file, cb) => {
    if (/audio|webm|ogg|mp3|wav/.test(file.mimetype)) cb(null, true);
...
app.put('/api/reviews/:id', auth, (req, res) => {
  try {
    ...
    if (i === -1) return res.status(404).json({ error: 'نظر یافت نشد' });
    
    // فقط صاحب نظر می‌تواند ویرایش کند
    if (reviews[i].userId !== req.user.id) {
      return res.status(403).json({ error: 'شما فقط می‌توانید نظرات خود را ویرایش کنید' });
    }
    ...
    writeJSON('reviews.json', reviews);
    console.log(`✏️ نظر ویرایش شد: ${req.params.id}`);
    res.json(reviews[i]);
  } catch (e) {
    res.status(500).json({ error: 'خطا در ویرایش نظر' });
  }
});
...
app.delete('/api/reviews/:id', auth, (req, res) => {
  try {
    ...
    // صاحب نظر یا ادمین می‌توانند حذف کنند
    if (reviews[i].userId !== req.user.id && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'فقط صاحب نظر یا مدیر می‌توانند این نظر را حذف کنند' });
    }
    
    const deleted = reviews.splice(i, 1)[0];
    writeJSON('reviews.json', reviews);
    console.log(`🗑️ نظر حذف شد: ${req.params.id} توسط ${req.user.role === 'admin' ? 'ادمین' : 'صاحب نظر'}`);
    res.json({ success: true });
  } catch (e) {
...
app.post('/api/chats/:chatId/messages', auth, (req, res) => {
  try {
    ...
    // پیام صوتی: یا voiceUrl از آپلود، یا voiceData (base64)
    let voiceUrl = req.body.voiceUrl || null;
    const voiceData = req.body.voiceData || null;
    if (req.body.type === 'voice' && !voiceUrl && voiceData && voiceData.startsWith('data:audio')) {
      const matches = voiceData.match(/^data:audio\/([a-zA-Z0-9]+);base64,(.+)$/);
      if (matches) {
        const ext = matches[1] === 'webm' ? 'webm' : 'mp3';
        const filename = `voice-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.${ext}`;
        const filepath = path.join(VOICE_UPLOADS, filename);
        fs.writeFileSync(filepath, Buffer.from(matches[2], 'base64'));
        voiceUrl = `${req.protocol}://${req.get('host')}/uploads/voice/${filename}`;
        console.log(`🎤 ذخیره فایل صوتی: ${filename}`);
      }
    }
    ...
    // پاک کردن وضعیت تایپ
    typingStatus.delete(`${req.params.chatId}-${req.user.id}`);
    
    writeJSON('chats.json', chats);
    
    const logContent = message.type === 'voice' ? '🎤 پیام صوتی' : message.content.substring(0, 30);
    console.log(`💬 پیام جدید در ${chat.id}: ${logContent}...`);
...
// آپلود پیام صوتی
app.post('/api/chats/:chatId/voice', auth, uploadVoice.single('voice'), (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'فایل صوتی یافت نشد' });
    
    const voiceUrl = `/uploads/voice/${req.file.filename}`;
    const fullUrl = `${req.protocol}://${req.get('host')}${voiceUrl}`;
    ...
    console.error('❌ خطا آپلود صدا:', e);
    res.status(500).json({ error: 'خطا در آپلود صدا' });
  }
});

// وضعیت تایپ
app.post('/api/chats/:chatId/typing', auth, (req, res) => {
  const { isTyping } = req.body;
  const key = `${req.params.chatId}-${req.user.id}`;
...
app.get('/api/chats/:chatId/typing', auth, (req, res) => {
  ...
  // پاک کردن تایپ‌های قدیمی (بیش از 5 ثانیه)
  for (const [key, status] of typingStatus.entries()) {
    if (now - status.timestamp > 5000) {
      typingStatus.delete(key);
    } else if (status.chatId === chatId && status.userId !== req.user.id) {
      typing.push(status);
    }
  }
...
// علامت تحویل پیام
app.put('/api/chats/:chatId/delivered', auth, (req, res) => {
  const chats = readJSON('chats.json');
  const chat = chats.find(c => c.id === req.params.chatId);
  if (!chat) return res.status(404).json({ error: 'چت یافت نشد' });
  
  // علامت‌گذاری پیام‌های طرف مقابل به عنوان تحویل داده شده
  chat.messages.forEach(m => {
    if (m.senderId !== req.user.id && !m.delivered) {
      m.delivered = true;
    }
  });
  writeJSON('chats.json', chats);
...
function findFrontendDir() {
  const candidates = [
    path.join(__dirname, 'scr', 'dist'),           // backend/public
